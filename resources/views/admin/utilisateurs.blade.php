...
@section('content')
    <div class="container admin-page">
        <div class="page-header">
            ...
            <a href="{{ route('admin.dashboard') }}" class="btn btn-outline btn-sm"><i class="fas fa-arrow-left"></i>
                Dashboard</a>
        </div>
        ...
        <div class="table-card">
            <table>
                ...
                <tbody>
                            ...
                            <td>
                                <div style="display:flex;align-items:center;gap:0.75rem;">
                                    ...
                                    @if ($u->isParticulier() && $u->particulier?->photo)
                                        <img src="{{ asset('storage/' . $u->particulier->photo) }}"
                                    ...
                                    @elseif($u->isEntreprise() && $u->entreprise?->logo)
                                        <img src="{{ asset('storage/' . $u->entreprise->logo) }}"
                                            style="width:60px;height:60px;border-radius:8px;object-fit:cover;border:2px solid var(--border);">
                                    @else
                                        <div
                                            style="width:60px;height:60px;border-radius:8px;background:var(--ink);color:var(--paper);display:flex;align-items:center;justify-content:center;font-family:var(--font-head);font-weight:800;font-size:0.85rem;flex-shrink:0;">
                            ...
                            <td>
                                @if ($u->role !== 'bloque' && $u->id !== auth()->id())
                                    <form method="POST" action="{{ route('admin.utilisateurs.bloquer', $u->id) }}"
                                        ...
                                        @csrf @method('PATCH')
                                        <button type="submit" class="btn btn-danger btn-sm"><i class="fas fa-ban"></i>
                                            Bloquer</button>
                                    </form>
                                @else
                    ...
                    @empty
                        <tr>
                            <td colspan="4" style="text-align:center;padding:2rem;color:var(--muted);">Aucun utilisateur.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div style="margin-top:1.5rem;display:flex;justify-content:center;">{{ $utilisateurs->withQueryString()->links() }}
        </div>
    </div>
@endsection
