...
<style>
    ...
    .form-page-header h1 { font-family: var(--font-head); font-size: 2rem; font-weight: 800; letter-spacing:-0.5px; }
    .form-page-header p { color: var(--muted); margin-top: 0.25rem; }

    .form-layout { display: grid; grid-template-columns: 1fr 320px; gap: 2rem; align-items: start; }
    ...
    .tips-card h4 { font-family: var(--font-head); font-size: 1rem; font-weight: 700; margin-bottom: 1rem; color: var(--paper); }

    .tip-item { display: flex; gap: 0.75rem; margin-bottom: 1rem; }
    .tip-item .tip-icon { font-size: 1.1rem; flex-shrink: 0; margin-top: 0.1rem; }
    .tip-item p { font-size: 0.83rem; color: #aaa; line-height: 1.5; }
    .tip-item p strong { color: var(--paper); display: block; margin-bottom: 0.15rem; }
...
<div class="container form-page">
    ...
    <div class="form-layout">
        <div>
            <form method="POST" action="{{ isset($offre) ? route('entreprise.offres.update', $offre->id) : route('entreprise.offres.store') }}">
                @csrf
                @if(isset($offre)) @method('PUT') @endif

                <!-- Informations principales -->
                <div class="form-card" style="margin-bottom:1.25rem;">
                    <h3><i class="fas fa-info-circle" style="color:var(--accent);margin-right:0.5rem;"></i> Informations principales</h3>

                    <div class="form-group">
                        <label>Titre du poste <span style="color:var(--accent)">*</span></label>
                        <input type="text" name="titre" class="form-control @error('titre') is-invalid @enderror"
                               value="{{ old('titre', $offre->titre ?? '') }}" placeholder="Ex: Développeur Full-Stack Senior" required>
                        ...
                        <div class="form-group">
                            ...
                            <select name="contrat" class="form-control">
                                ...
                                @foreach(['CDI','CDD','Stage','Alternance','Freelance','Intérim'] as $type)
                                    <option value="{{ $type }}" {{ old('contrat', $offre->contrat ?? '') == $type ? 'selected' : '' }}>{{ $type }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <!-- Conditions -->
                <div class="form-card" style="margin-bottom:1.25rem;">
                    <h3><i class="fas fa-list-ul" style="color:var(--accent);margin-right:0.5rem;"></i> Conditions du poste</h3>
                        ...
                        <div class="form-group">
                            ...
                            <select name="niveau_etude" class="form-control">
                                ...
                                @foreach(['Bac','Bac+2','Bac+3','Bac+5','Doctorat','Sans diplôme'] as $niv)
                                    <option value="{{ $niv }}" {{ old('niveau_etude', $offre->niveau_etude ?? '') == $niv ? 'selected' : '' }}>{{ $niv }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
        ...
        <div class="tips-card">
            ...
            <div class="tip-item">
                <span class="tip-icon">📋</span>
                <p><strong>Description complète</strong>Listez les missions, les compétences requises et les avantages offerts.</p>
            </div>
            <div class="tip-item">
                <span class="tip-icon">💰</span>
