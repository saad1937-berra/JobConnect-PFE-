        ...
        <div class="matching-layout">
            ...
            <div>
                ...
                <div class="score-card">
                    ...
                    <div class="score-circle">
                        <svg width="160" height="160" viewBox="0 0 160 160">
                            <!-- Fond -->
                            <circle cx="80" cy="80" r="65" fill="none" stroke="#f0ede6"
                                stroke-width="12" />
                            <!-- Jauge -->
                            <circle cx="80" cy="80" r="65" fill="none" stroke="{{ $couleur }}"
                                stroke-width="12" stroke-linecap="round" stroke-dasharray="{{ round(2 * M_PI * 65) }}"
                    ...
                    <div class="score-desc">
                        @if ($score >= 80)
                            Excellent profil pour cette offre — postulez dès maintenant !
                        @elseif($score >= 60)
                            Bon profil — vous correspondez bien à cette offre.
                        @elseif($score >= 40)
                            Profil moyen — quelques points à améliorer.
                        @else
                            Profil peu compatible — consultez d'autres offres.
                        @endif
                    </div>
                ...
                <div class="criteres-card">
                    <h3><i class="fas fa-chart-bar" style="color:var(--accent);margin-right:0.5rem;"></i> Détail du score
                    </h3>
                        ...
                        <div class="critere-item">
                            ...
                            <div class="progress-bar">
                                <div class="progress-fill"
                                    style="width:{{ $pct }}%;background:{{ $pct >= 80 ? '#28a745' : ($pct >= 50 ? '#17a2b8' : ($pct >= 30 ? '#ffc107' : '#dc3545')) }};">
                                </div>
                            </div>
                            ...
                            @if ($key === 'competences' && isset($critere['detail_niveau']) && count($critere['detail_niveau']) > 0)
                                <div class="critere-detail">
                                    @foreach ($critere['detail_niveau'] as $item)
                                        @php
                                            $niveauColor = match ($item['niveau']) {
                                                'Expert' => '#28a745',
                                                'Avancé' => '#17a2b8',
                                                'Intermédiaire' => '#f0a500',
                                                default => '#888',
                                            };
                                            $poidsPct = round($item['poids'] * 100);
                                        @endphp
                                        <span
                                            style="
            ...
            ">
                                            ...
                                            {{ $item['nom'] }}
                                            <span
                                                style="background:{{ $niveauColor }};color:white;padding:0.05rem 0.35rem;border-radius:8px;font-size:0.65rem;">
                                                {{ $item['niveau'] }} ({{ $poidsPct }}%)
                                            </span>
                                        </span>
                                    @endforeach
                                </div>
                            @elseif(isset($critere['detail']) && is_array($critere['detail']) && count($critere['detail']) > 0)
                                <div class="critere-detail">
                            ...
                            @elseif(isset($critere['note']) && $critere['note'])
                                <div class="critere-detail">{{ $critere['note'] }}</div>
                            @endif
                        </div>
                    @endforeach
                </div>
                ...
                @if ($score < 80)
                    <div
                        style="background:white;border:1px solid var(--border);border-radius:var(--radius);padding:1.75rem;">
                        <h3 style="font-family:var(--font-head);font-size:1rem;font-weight:700;margin-bottom:1.25rem;">
                            <i class="fas fa-lightbulb" style="color:#ffc107;margin-right:0.5rem;"></i> Comment améliorer
                            votre score
                        </h3>
                        @if ($matching['criteres']['competences']['score'] < 30)
                            <div style="display:flex;gap:0.75rem;align-items:flex-start;margin-bottom:0.85rem;">
                                ...
                                <div>
                                    ...
                                    <p style="font-size:0.82rem;color:var(--muted);">Plus vous avez de compétences
                                        correspondantes, meilleur est votre score.</p>
                                </div>
                            </div>
                        @endif
                        @if ($matching['criteres']['profil']['score'] < 8)
                            <div style="display:flex;gap:0.75rem;align-items:flex-start;margin-bottom:0.85rem;">
                                ...
                                <div>
                                    ...
                                    <p style="font-size:0.82rem;color:var(--muted);">Ajoutez votre bio, téléphone et
                                        uploadez votre CV.</p>
                                </div>
                            </div>
                        @endif
                        @if ($matching['criteres']['localisation']['score'] < 15)
                            <div style="display:flex;gap:0.75rem;align-items:flex-start;">
                                ...
                                <div>
                                    ...
                                    <p style="font-size:0.82rem;color:var(--muted);">Une localisation précise améliore votre
                                        score de matching.</p>
                                </div>
                            </div>
                        @endif
            ...
            <div>
                <div class="offre-card">
                    ...
                    <div class="info-row"><span class="lbl"><i class="fas fa-briefcase"></i> Contrat</span><span
                            class="val">{{ $offre->contrat ?? '—' }}</span></div>
                    <div class="info-row"><span class="lbl"><i class="fas fa-map-marker-alt"></i> Lieu</span><span
                            class="val">{{ $offre->localisation ?? '—' }}</span></div>
                    <div class="info-row"><span class="lbl"><i class="fas fa-graduation-cap"></i> Niveau</span><span
                            class="val">{{ $offre->niveau_etude ?? '—' }}</span></div>
                    <div class="info-row"><span class="lbl"><i class="fas fa-money-bill-wave"></i> Salaire</span><span
                            class="val">{{ $offre->salaire ?? 'Non précisé' }}</span></div>
                    <div class="info-row"><span class="lbl"><i class="fas fa-clock"></i> Durée</span><span
                            class="val">{{ $offre->duree ?? '—' }}</span></div>
                </div>

                <!-- Mon profil recap -->
                <div class="offre-card">
                    <h3>Mon profil</h3>
                    <div class="info-row"><span class="lbl">Compétences</span><span
                            class="val">{{ $particulier->competances->count() }}</span></div>
                    <div class="info-row"><span class="lbl">CV</span><span
                            class="val">{{ $particulier->cv->count() > 0 ? '✅ Disponible' : '❌ Manquant' }}</span>
                    </div>
                    <div class="info-row"><span class="lbl">Localisation</span><span
                            class="val">{{ $particulier->adresse ?? 'Non renseignée' }}</span></div>
                    <div class="info-row"><span class="lbl">Bio</span><span
                            class="val">{{ !empty($particulier->bio) ? '✅' : '❌' }}</span></div>
                </div>

                <a href="{{ route('particulier.matching') }}" class="btn btn-outline"
